...
let main () = 
  ...
  | "export" ->     
    ...
    ignore(trash)
  | "ami" -> 
    let oc = open_out ("results/metadata.ami_results.txt")
  ...
  | "vox" | "libri" | "vy" | "wsj" -> 
    ...
    | "vy" -> (find_words cmdlist accesstext_vy accesswav_vy dirpath false, 
      "vy_results.txt")
    | "wsj" -> print_endline "wsj"; (wsj0_dict dirpath cmdlist, "wsj_results.txt")) in
    let cmd_dict = make_cmd_dict res D.empty in
    let oc = open_out ("results/" ^ txtout) in
    print_result oc cmd_dict
  | _ -> failwith (Invalid_argument "argv 1 is invalid")
  ;;
